...
export const getBalances = createAsyncThunk(
  ...
  async ({ address, networkID, provider }: IBaseAddressAsyncThunk) => {
    const ohmContract = new ethers.Contract(addresses[networkID].SPACE_ADDRESS as string, ierc20Abi, provider);
    const ohmBalance = await ohmContract.balanceOf(address);
    const sohmContract = new ethers.Contract(addresses[networkID].SSPACE_ADDRESS as string, ierc20Abi, provider);
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    if (addresses[networkID].SPACE_ADDRESS) {
      const ohmContract = new ethers.Contract(addresses[networkID].SPACE_ADDRESS as string, ierc20Abi, provider);
      ohmBalance = await ohmContract.balanceOf(address);
      stakeAllowance = await ohmContract.allowance(address, addresses[networkID].STAKING_HELPER_ADDRESS);
    }
    
    if (addresses[networkID].SSPACE_ADDRESS) {
      const sohmContract = new ethers.Contract(addresses[networkID].SSPACE_ADDRESS as string, sOHMv2, provider);
      sohmBalance = await sohmContract.balanceOf(address);
      unstakeAllowance = await sohmContract.allowance(address, addresses[networkID].STAKING_ADDRESS);
    }
   
    if (addresses[networkID].WSSPACE_ADDRESS) {
      const wsohmContract = new ethers.Contract(addresses[networkID].WSSPACE_ADDRESS as string, wsOHM, provider);
