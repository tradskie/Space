...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const ohmContract = new ethers.Contract(addresses[networkID].SPACE_ADDRESS as string, ierc20Abi, signer);
    const sohmContract = new ethers.Contract(addresses[networkID].SSPACE_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
    try {
      if (token === "ohm") {
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    try {
      if (action === "stake") {
        ...
        uaData.type = "warmupclaim";
        stakeTx = await staking.claim(address);
      }
      const pendingTxnType = action === "stake" ? "staking" : action === "unstake" ? "unstaking" : "warmupclaiming";
