      ...
      {!address ? (
        ...
        <Box className="withdrawal-container">
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            >
              {exitFee > 0
                ? txnButtonText(pendingTransactions, "pool_withdraw", "Withdraw Early & pay " + exitFee + " sSPACE")
                : txnButtonText(pendingTransactions, "pool_withdraw", "Withdraw sSPACE")}
              {/* Withdraw sSPACE */}
            </Button>
          </Box>
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography color="error" variant="body2">
                Withdrawing {quantity} sSPACE reduces your odds of winning to 1 in {newOdds}&nbsp;
              </Typography>
            </Box>
          )}
          {exitFee > 0 && (
