...
import { space_busd } from "./AllBonds";
...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const space_busd_address = space_busd.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(space_busd_address, PairContract, provider);
  const reserves = await pairContract.getReserves();
  const marketPrice = (Number(reserves[1]))/Number(reserves[0]) / Math.pow(10, 9);
  return marketPrice;
}

export async function getTokenPrice(tokenId = "SPACE") {
  console.error("GetTokenPrice");
